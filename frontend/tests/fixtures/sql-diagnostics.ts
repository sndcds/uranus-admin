...
export const diagnosticDefinition: SqlDiagnosticDefinition = {
  ...
  datasource: 'uranus',
  readonly: true,
  sql: 'SELECT uuid, point IS NULL AS point_missing FROM uranus.venue WHERE uuid = :entity_key LIMIT :diagnostic_limit',
  copy_sql:
    "SELECT uuid, point IS NULL AS point_missing FROM uranus.venue WHERE uuid = '00000000-0000-0000-0000-000000000020' LIMIT 50;",
  parameters: { entity_key: '00000000-0000-0000-0000-000000000020', diagnostic_limit: 50 },
...
export const diagnosticResult: SqlDiagnosticResult = {
  recipe_id: 'venue_missing_location',
  columns: ['uuid', 'point_missing'],
  rows: [{ uuid: '00000000-0000-0000-0000-000000000020', point_missing: true }],
  row_count: 1,
  duration_ms: 1.5,
