...
test('assign, snooze until tomorrow and unsnooze through the Inbox', async ({ page }) => {
  ...
  await page.goto('/inbox')
  await expect(page.getByRole('button', { name: '1 Wiedervorlagen', exact: true })).toBeVisible()
  ...
  await expect(
    page.locator('.data-row p').filter({ hasText: 'Rückkehr in die Inbox' }).locator('time'),
  ).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Anwenden', exact: true }).click()
  await expect(page.locator('.data-row')).toHaveCount(1)
  await expect(page.getByRole('button', { name: '0 Wiedervorlagen', exact: true })).toBeVisible()
...
test('a finding review snooze has one task and directs changes to the finding', async ({
  ...
  await expect(page.locator('.data-row')).toHaveCount(1)
  await expect(page.getByText(/Fachliche Zurückstellung/)).toBeVisible()
  await expect(page.getByRole('link', { name: 'Befund ansehen' })).toBeVisible()
