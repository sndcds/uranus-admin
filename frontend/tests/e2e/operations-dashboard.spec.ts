...
for (const [name, width, height] of [
  ...
  test(`operations dashboard ${name}`, async ({ page }, info) => {
    ...
    await page.keyboard.press('Enter')
    await expect(page.getByRole('combobox', { name: 'Schweregrad', exact: true })).not.toBeVisible()
    const more = worklist.locator('summary').nth(2)
    await more.click()
    await expect(worklist.getByRole('button', { name: 'SQL Editor für Hafenbühne' })).toBeVisible()
    await expect(
      worklist.getByRole('link', { name: 'Markierungen & Notizen' }).filter({ visible: true }),
    ).toBeVisible()
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
    await more.click()
    const detail = worklist.getByRole('button', { name: 'Befund zu Hafenbühne ansehen' })
    await detail.click()
    await expect(page.getByRole('dialog', { name: 'Hafenbühne' })).toBeVisible()
