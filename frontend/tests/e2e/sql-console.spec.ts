...
test('console: same-origin relay, formatting, results, error, cancel and screenshots', async ({
  ...
  await page.goto('/sql')
  await expect(page.locator('.sql-workspace strong').filter({ hasText: /^Uranus$/ })).toHaveCount(1)
  await expect(page.locator('.sql-workspace').getByText('uranus.*', { exact: true })).toBeVisible()
  ...
  await expect(page.locator('.sql-workspace').getByText('getrennt', { exact: true })).toBeVisible()
  ...
  await expect(page.locator('.sql-workspace').getByText('verbunden', { exact: true })).toBeVisible()
